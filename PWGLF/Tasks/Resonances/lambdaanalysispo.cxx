...
struct Lambda1520Analysispo {
  ...
  Configurable<float> minKaonMomentum{"minKaonMomentum", 0.f,"Minimum total momentum p for kaon PID [GeV/c]"};
  ...
  Configurable<bool> requirePrimaryTrack{"requirePrimaryTrack", true,"Require track to pass the 'isPrimaryTrack' flag (kGoldenChi2 | kDCAxy | kDCAz)"};
  Configurable<bool> requireGlobalTrackNoDCA{"requireGlobalTrackNoDCA", true,"Require track to pass 'isGlobalTrackWoDCA' (quality cuts without DCA requirement)"};
  Configurable<bool> requirePVContributor{"requirePVContributor", true,"Require track to be a contributor to the primary vertex reconstruction"};
  ...
  Configurable<bool> requireTOFForKaon{"requireTOFForKaon", false,"If true, only accept kaon candidates that have a TOF measurement"};
  Configurable<bool> useTPCOnlyPID{"useTPCOnlyPID", false,"If true, use only TPC nSigma for PID (ignore TOF even if available)"};

  Configurable<float> tpcNSigmaVetoOtherSpecies{"tpcNSigmaVetoOtherSpecies", 3.0f,"Reject track if its TPC nSigma for a different species is below this threshold (avoids misID)"};
  Configurable<float> tpcNSigmaVetoPion{"tpcNSigmaVetoPion", 3.0f,"TPC nSigma threshold below which a track is vetoed as a pion"};
  Configurable<float> tpcNSigmaVetoKaonForProton{"tpcNSigmaVetoKaonForProton", 3.0f,"TPC nSigma threshold: veto proton candidates that look like kaons"};
  Configurable<float> tpcNSigmaVetoPionForKaon{"tpcNSigmaVetoPionForKaon", 3.0f,"TPC nSigma threshold: veto kaon candidates that look like pions"};
  Configurable<float> tpcNSigmaVetoProtonForKaon{"tpcNSigmaVetoProtonForKaon", 3.0f,"TPC nSigma threshold: veto kaon candidates that look like protons"};
  ...
  Configurable<float> minCombinedTPCTOFNSigmaKaon{"minCombinedTPCTOFNSigmaKaon", -6.0f,"Minimum combined TPC+TOF nSigma for kaon (used in combined PID mode)"};
  Configurable<float> minCombinedTPCTOFNSigmaProton{"minCombinedTPCTOFNSigmaProton", -6.0f,"Minimum combined TPC+TOF nSigma for proton (used in combined PID mode)"};

  Configurable<float> tpcNSigmaVetoThreshold{"tpcNSigmaVetoThreshold", 3.0f,"General TPC nSigma veto cut to reject misidentified particles when TPC bands overlap"};
  ...
  Configurable<double> maxTPCNSigmaProton{"maxTPCNSigmaProton", 3.0,"Maximum |TPC nSigma| for proton identification (symmetric cut)"};
  ...
  Configurable<std::vector<float>> protonTPCPIDMomentumBins{"protonTPCPIDMomentumBins", {0, 0.5, 0.7, 0.8},"Momentum p bin edges [GeV/c] for momentum-dependent TPC PID cuts on protons"};
  Configurable<std::vector<float>> protonTPCNSigmaCutPerBin{"protonTPCNSigmaCutPerBin",{5., 3.5, 2.5},"Maximum TPC nSigma for proton in each momentum bin (tighter at higher p)"};
  Configurable<std::vector<float>> protonTOFPIDMomentumBins{"protonTOFPIDMomentumBins",{0., 999.},"Momentum p bin edges [GeV/c] for momentum-dependent TOF PID cuts on protons"};
  ...
  Configurable<double> maxTPCNSigmaKaon{"maxTPCNSigmaKaon", 3.0,"Maximum |TPC nSigma| for kaon identification (symmetric cut)"};
  Configurable<double> combinedNSigmaCutKaon{"combinedNSigmaCutKaon", 3.0,"Cut on sqrt(nSigmaTPC^2 + nSigmaTOF^2) for kaon. Negative value switches to asymmetric mode."};
  Configurable<std::vector<float>> kaonTPCPIDMomentumBins{"kaonTPCPIDMomentumBins", {0., 0.25, 0.3, 0.45},"Momentum p bin edges [GeV/c] for momentum-dependent TPC PID cuts on kaons"};
  Configurable<std::vector<float>> kaonTPCNSigmaCutPerBin{"kaonTPCNSigmaCutPerBin",{6, 3.5, 2.5},"Maximum TPC nSigma for kaon in each momentum bin"};
  Configurable<std::vector<float>> kaonTOFPIDMomentumBins{"kaonTOFPIDMomentumBins",{0., 999.},"Momentum p bin edges [GeV/c] for momentum-dependent TOF PID cuts on kaons"};
  Configurable<std::vector<float>> kaonTOFNSigmaCutPerBin{"kaonTOFNSigmaCutPerBin",{3.0},"Maximum TOF nSigma for kaon in each momentum bin"};

  // ── Event mixing configurables ───────────────────────────────────────────
  Configurable<int> numberOfEventsToMix{"numberOfEventsToMix", 20,"Number of events to mix with each signal event for background estimation"};
  ConfigurableAxis dcaZMixingBins{"dcaZMixingBins",{VARIABLE_WIDTH, -1.2f, -1.0f, -0.9f, -0.8f, -0.7f, -0.6f, -0.5f,-0.4f, -0.3f, -0.2f, -0.1f, 0.f, 0.1f, 0.2f, 0.3f,0.4f, 0.5f, 0.6f, 0.7f, 0.8f, 0.9f, 1.0f, 1.2f},"DCAz vertex bins used for event mixing pairing"};
  ConfigurableAxis vertexZMixingBins{"vertexZMixingBins",{VARIABLE_WIDTH, -12.f, -10.f, -9.f, -8.f, -7.f, -6.f, -5.f, -4.f, -3.f, -2.f, -1.f, 0.f, 1.f, 2.f, 3.f, 4.f, 5.f, 6.f, 7.f, 8.f, 9.f, 10.f, 12.f},"Primary vertex z-position bins used for event mixing pairing"};
  ConfigurableAxis centralityMixingBins{"centralityMixingBins",{VARIABLE_WIDTH, 0.f, 10.f, 20.f, 30.f, 40.f, 50.f, 60.f, 70.f, 80.f,90.f, 100.f, 200.f},"Centrality (FT0 %) bins used for event mixing pairing"};
  ConfigurableAxis eventPlaneMixingBins{"eventPlaneMixingBins",{VARIABLE_WIDTH, -1.5708f, -1.25664f, -0.942478f, -0.628319f, 0.f,0.628319f, 0.942478f, 1.25664f, 1.5708f},"Event plane angle bins used for event mixing (EP-dependent analysis)"};
  ConfigurableAxis occupancyBins{"occupancyBins",{VARIABLE_WIDTH, 0.0, 100, 500, 600, 1000, 1100, 1500,1600, 2000, 2100, 2500, 2600, 3000, 3100, 3500,3600, 4000, 4100, 4500, 4600, 5000, 5100, 9999},"Track occupancy bins in the time range around the collision"};

  // ── Rotational background configurables ─────────────────────────────────
  Configurable<int> numberOfRotations{"numberOfRotations", 10,"How many times to rotate the kaon phi for the rotational background estimate"};
  Configurable<float> rotationAngleWindow{"rotationAngleWindow", 6.f,"The kaon is rotated by angles near PI, within a window of PI/rotationAngleWindow"};
  ...
  void init(InitContext const&)
  {
    ...
    const AxisSpec axisInvariantMass(numberOfInvMassBins, 1.4, 2.0,"M_{inv} (GeV/c^{2})");
    ...
    if (doprocessMix || doprocessMixDF || doprocessMixepDF) {
      allHistograms.add("Event/mixingBins_centralityVsVtxZVsEventPlane","Event mixing bin occupancy: centrality vs vtxZ vs event plane",kTH3F, {axisCentralityPercent, axisVertexZ, axisEventPlaneAngle});
    }
    ...
    allHistograms.add("QAbefore/Kaon/tpcNSigmaVsMomentum","TPC nSigma kaon vs momentum (before PID cuts)", kTH2F,{axisMomentumForPID, axisTPCNSigma});
    allHistograms.add("QAbefore/Kaon/tofNSigmaVsMomentum","TOF nSigma kaon vs momentum (before PID cuts)", kTH2F,{axisMomentumForPID, axisTOFNSigma});
    allHistograms.add("QAbefore/Kaon/tofNSigmaVsTPCNSigma","TOF nSigma vs TPC nSigma kaon (before PID cuts)", kTH2F,{axisTPCNSigma, axisTOFNSigma});
    ...
    allHistograms.add("QAafter/Kaon/ptVsCentrality","Kaon pT vs centrality (after cuts)", kTH2F,{axisPtForPID, axisCentralityPercent});
    allHistograms.add("QAafter/Kaon/dcaZVsPt", "Kaon DCAz vs pT (after cuts)",kTH2F, {axisPtForPID, axisDCAz});
    allHistograms.add("QAafter/Kaon/dcaXYVsPt", "Kaon DCAxy vs pT (after cuts)",kTH2F, {axisPtForPID, axisDCAxy});
    allHistograms.add("QAafter/Kaon/tpcDedxVsMomentum","Kaon TPC dE/dx signal vs momentum (after cuts)", kTH2F,{axisMomentumForPID, axisTPCdEdx});
    allHistograms.add("QAafter/Kaon/tpcNSigmaPionContamVsPt","Kaon track: TPC nSigma pion contamination check", kTH2F,{axisPtForPID, axisTPCNSigma});
    ...
    allHistograms.add("QAafter/Kaon/tpcCrossedRowsVsPt","Kaon TPC crossed rows vs pT", kTH2F,{axisPtForPID, {200, 0, 200}});
    allHistograms.add("QAafter/Kaon/tpcClustersFoundVsPt","Kaon TPC clusters found vs pT", kTH2F,{axisPtForPID, {200, 0, 200}});

    if (!doprocessMC) {
      allHistograms.add("Analysis/invMass_UnlikeSign_ProtonPlusKaonMinus","Invariant mass: p^{+}K^{-} (Lambda(1520) signal)", kTHnSparseF,{axisInvariantMass, axisPt, axisCentralityPercent, axisOccupancy});
      allHistograms.add("Analysis/invMass_UnlikeSign_ProtonMinusKaonPlus","Invariant mass: p^{-}K^{+} (anti-Lambda(1520) signal)", kTHnSparseF,{axisInvariantMass, axisPt, axisCentralityPercent, axisOccupancy});
      allHistograms.add("Analysis/invMass_LikeSign_ProtonPlusKaonPlus","Invariant mass: p^{+}K^{+} (like-sign background)", kTHnSparseF,{axisInvariantMass, axisPt, axisCentralityPercent, axisOccupancy});
      allHistograms.add("Analysis/invMass_LikeSign_ProtonMinusKaonMinus","Invariant mass: p^{-}K^{-} (like-sign background)", kTHnSparseF,{axisInvariantMass, axisPt, axisCentralityPercent, axisOccupancy});
      allHistograms.add("Analysis/invMass_Rotated_ProtonPlusKaonMinus","Invariant mass: rotational background (Lambda(1520))", kTHnSparseF,{axisInvariantMass, axisPt, axisCentralityPercent, axisOccupancy});
      allHistograms.add("Analysis/invMass_Rotated_ProtonMinusKaonPlus","Invariant mass: rotational background (anti-Lambda(1520))",kTHnSparseF,{axisInvariantMass, axisPt, axisCentralityPercent, axisOccupancy});
      allHistograms.add("Analysis/invMass_Mixed_ProtonPlusKaonMinus","Invariant mass: mixed events p^{+}K^{-} (background)", kTHnSparseF,{axisInvariantMass, axisPt, axisCentralityPercent, axisOccupancy});
      allHistograms.add("Analysis/invMass_Mixed_ProtonMinusKaonPlus","Invariant mass: mixed events p^{-}K^{+} (background)", kTHnSparseF,{axisInvariantMass, axisPt, axisCentralityPercent, axisOccupancy});
      allHistograms.add("Analysis/invMass_Mixed_LikeSign_PlusPlus","Invariant mass: mixed events like-sign ++", kTHnSparseF,{axisInvariantMass, axisPt, axisCentralityPercent, axisOccupancy});
      allHistograms.add("Analysis/invMass_Mixed_LikeSign_MinusMinus","Invariant mass: mixed events like-sign --", kTHnSparseF,{axisInvariantMass, axisPt, axisCentralityPercent, axisOccupancy});
    }

    if (doprocessMC) {
      allHistograms.add("Event/mcEventSelectionCutflow","MC event selection cutflow (how many events pass each cut)", kTH1F,{{7, 0, 7}});
      ...
      allHistograms.add("Analysis/mcGenerated_Lambda1520","Generated Lambda(1520) invariant mass vs pT vs centrality",kTHnSparseF, {axisInvariantMass, axisPt, axisCentralityPercent});
      allHistograms.add("Analysis/mcGenerated_AntiLambda1520","Generated anti-Lambda(1520) invariant mass vs pT vs centrality",kTHnSparseF, {axisInvariantMass, axisPt, axisCentralityPercent});
      allHistograms.add("Analysis/mcReconstructed_Lambda1520","Reconstructed Lambda(1520) invariant mass vs pT vs centrality - MC",kTHnSparseF, {axisInvariantMass, axisPt, axisCentralityPercent});
      allHistograms.add("Analysis/mcReconstructed_AntiLambda1520","Reconstructed anti-Lambda(1520) invariant mass vs pT vs centrality - MC",kTHnSparseF,{axisInvariantMass, axisPt, axisCentralityPercent});
      allHistograms.add("Analysis/mcMassResolution_Lambda1520","Mass resolution (Mreco - Mgen) vs pT - Lambda(1520)",kTHnSparseF,{{200, -0.05, 0.05}, axisPt, axisCentralityPercent});
      allHistograms.add("Analysis/mcMassResolution_AntiLambda1520","Mass resolution (Mreco - Mgen) vs pT - anti-Lambda(1520)",kTHnSparseF, {{200, -0.05, 0.05}, axisPt, axisCentralityPercent});
    }

    if (doprocessMCGen) {
      allHistograms.add("SignalLoss/mcEventSelectionCutflow","MC event selection cutflow (signal loss study)", kTH1F,{{7, 0, 7}});
      allHistograms.add("SignalLoss/mTScaled_fromProton","mT-scaled Lambda(1520) pT from proton parent",kTHnSparseF, {axisPt, axisCentralityPercent});
      allHistograms.add("SignalLoss/mTScaled_fromAntiProton","mT-scaled anti-Lambda(1520) pT from anti-proton parent", kTHnSparseF,{axisPt, axisCentralityPercent});
      allHistograms.add("SignalLoss/mTScaled_fromLambda0","mT-scaled Lambda(1520) pT from Lambda0 parent",kTHnSparseF, {axisPt, axisCentralityPercent});
      allHistograms.add("SignalLoss/mTScaled_fromAntiLambda0","mT-scaled anti-Lambda(1520) pT from anti-Lambda0 parent",kTHnSparseF, {axisPt, axisCentralityPercent});
      allHistograms.add("SignalLoss/mTScaled_fromXiMinus","mT-scaled Lambda(1520) pT from Xi- parent",kTHnSparseF, {axisPt, axisCentralityPercent});
      allHistograms.add("SignalLoss/mTScaled_fromXiPlus","mT-scaled anti-Lambda(1520) pT from Xi+ parent",kTHnSparseF, {axisPt, axisCentralityPercent});
      allHistograms.add("SignalLoss/mTScaled_fromXi0","mT-scaled Lambda(1520) pT from Xi0 parent",kTHnSparseF, {axisPt, axisCentralityPercent});
      allHistograms.add("SignalLoss/mTScaled_fromAntiXi0","mT-scaled anti-Lambda(1520) pT from anti-Xi0 parent",kTHnSparseF, {axisPt, axisCentralityPercent});
      allHistograms.add("SignalLoss/mTScaled_fromOmegaMinus","mT-scaled Lambda(1520) pT from Omega- parent",kTHnSparseF, {axisPt, axisCentralityPercent});
      allHistograms.add("SignalLoss/mTScaled_fromOmegaPlus","mT-scaled anti-Lambda(1520) pT from Omega+ parent",kTHnSparseF, {axisPt, axisCentralityPercent});
    }
  }
  ...
  bool passesProtonDCASelection(TrackType const& track, float totalMomentum)
  {
    ...
    for (int iBin = 0; iBin < numberOfBins; iBin++) {
      if (totalMomentum >= ptBinEdges[iBin] &&
          totalMomentum < ptBinEdges[iBin + 1] &&
  ...
  bool passesProtonPID(TrackType const& track, float totalMomentum)
  {
    ...
    if (!useTPCOnlyPID && track.hasTOF()) {
      ...
      for (int i = 0; i < nTPCBins - 1; ++i) {
        if (totalMomentum >= tpcMomBins[i] &&
            totalMomentum < tpcMomBins[i + 1] &&
                                   ...
                                   int occupancyValue = 100)
  {
    ...
    for (auto const& [protonTrack, kaonTrack] :
      ...
      if (protonTrack.index() == kaonTrack.index())
        continue;
      if (!passesBasicTrackSelection(protonTrack) ||
          !passesBasicTrackSelection(kaonTrack))
        continue;
      ...
      float pairRapidity = RecoDecay::y(std::array{pxProton + pxKaon, pyProton + pyKaon, pzProton + pzKaon},pairInvMass);
      ...
      if constexpr (!isMixedEvent && !isMCAnalysis) {
        if (protonTrack.sign() * kaonTrack.sign() < 0) {
          if (protonTrack.sign() > 0)
            allHistograms.fill(HIST("Analysis/invMass_UnlikeSign_ProtonPlusKaonMinus"),pairInvMass, pairPt, centralityPercent, occupancyValue);
          else
            allHistograms.fill(HIST("Analysis/invMass_UnlikeSign_ProtonMinusKaonPlus"),pairInvMass, pairPt, centralityPercent, occupancyValue);
          ...
          if (enableRotationalBackground) {
            for (int iRot = 0; iRot < numberOfRotations; iRot++) {
              ...
              std::array<float, 3> momProtonRot = {pxProton, pyProton,
                                                   pzProton};
      ...
      if constexpr (isMCAnalysis) {
        if (protonTrack.sign() * kaonTrack.sign() < 0) {
          // FIX pdg/explicit-code: use named constants from lambda_analysis
          // namespace
          if (std::abs(protonTrack.pdgCode()) != lambda_analysis::PdgProton ||
              std::abs(kaonTrack.pdgCode()) != lambda_analysis::PdgKaon)
            continue;
                 ...
                 aod::ResoMCParents const& mcParents)
  {
    ...
    allHistograms.fill(HIST("Event/centralityVsOccupancy"), centralityPercent,
                       100);
    allHistograms.fill(HIST("Event/primaryVertexZ"), collision.posZ());
    ...
    for (const auto& track : tracks) {
      ...
      allHistograms.fill(HIST("QAbefore/trackEtaVsPhi"), track.eta(),
                         track.phi());

      // FIX pdg/explicit-code: use named PDG constants
      if (std::abs(track.pdgCode()) == lambda_analysis::PdgKaon)
        allHistograms.fill(HIST("QAChecks/kaonGenLevelPt"), track.pt());
      if (std::abs(track.pdgCode()) == lambda_analysis::PdgProton)
      ...
      if (passesKaonPID(track, totalMom) &&
          std::abs(track.pdgCode()) == lambda_analysis::PdgKaon)
        allHistograms.fill(HIST("QAChecks/kaonRecoLevelPt"), track.pt());
      if (passesProtonPID(track, totalMom) &&
          std::abs(track.pdgCode()) == lambda_analysis::PdgProton)
    ...
    for (const auto& parentParticle : mcParents) {
      ...
      if (parentParticle.pdgCode() > 0)
        allHistograms.fill(HIST("Analysis/mcGenerated_Lambda1520"), parentMass,
                           parentParticle.pt(), centralityPercent);
      else
                    ...
                    aod::ResoMCParents const& mcParents)
  {
    ...
    for (const auto& parentParticle : mcParents) {
      ...
      // (already done via using)
      auto computeMtScaledPtSquared = [&]() -> double {
        return (parentPt * parentPt) + (parentMass * parentMass) -
               (MassLambda1520 * MassLambda1520);
      };

      // FIX pdg/explicit-code: use named PDG constants from lambda_analysis
      // namespace
      if (pdgCode == lambda_analysis::PdgProton) {
        mTScaledPtSquared = computeMtScaledPtSquared();
        if (mTScaledPtSquared > 0)
          allHistograms.fill(HIST("SignalLoss/mTScaled_fromProton"),
                             std::sqrt(mTScaledPtSquared), centralityPercent);
      } else if (pdgCode == -lambda_analysis::PdgProton) {
        mTScaledPtSquared = computeMtScaledPtSquared();
        if (mTScaledPtSquared > 0)
          allHistograms.fill(HIST("SignalLoss/mTScaled_fromAntiProton"),
                             std::sqrt(mTScaledPtSquared), centralityPercent);
      } else if (pdgCode == lambda_analysis::PdgLambda0) {
        mTScaledPtSquared = computeMtScaledPtSquared();
        if (mTScaledPtSquared > 0)
          allHistograms.fill(HIST("SignalLoss/mTScaled_fromLambda0"),
                             std::sqrt(mTScaledPtSquared), centralityPercent);
      } else if (pdgCode == -lambda_analysis::PdgLambda0) {
        mTScaledPtSquared = computeMtScaledPtSquared();
        if (mTScaledPtSquared > 0)
          allHistograms.fill(HIST("SignalLoss/mTScaled_fromAntiLambda0"),
                             std::sqrt(mTScaledPtSquared), centralityPercent);
      } else if (pdgCode == lambda_analysis::PdgXiMinus) {
        mTScaledPtSquared = computeMtScaledPtSquared();
        if (mTScaledPtSquared > 0)
          allHistograms.fill(HIST("SignalLoss/mTScaled_fromXiMinus"),
                             std::sqrt(mTScaledPtSquared), centralityPercent);
      } else if (pdgCode == -lambda_analysis::PdgXiMinus) {
        mTScaledPtSquared = computeMtScaledPtSquared();
        if (mTScaledPtSquared > 0)
          allHistograms.fill(HIST("SignalLoss/mTScaled_fromXiPlus"),
                             std::sqrt(mTScaledPtSquared), centralityPercent);
      } else if (pdgCode == lambda_analysis::PdgXi0) {
        mTScaledPtSquared = computeMtScaledPtSquared();
        if (mTScaledPtSquared > 0)
          allHistograms.fill(HIST("SignalLoss/mTScaled_fromXi0"),
                             std::sqrt(mTScaledPtSquared), centralityPercent);
      } else if (pdgCode == -lambda_analysis::PdgXi0) {
        mTScaledPtSquared = computeMtScaledPtSquared();
        if (mTScaledPtSquared > 0)
          allHistograms.fill(HIST("SignalLoss/mTScaled_fromAntiXi0"),
                             std::sqrt(mTScaledPtSquared), centralityPercent);
      } else if (pdgCode == lambda_analysis::PdgOmegaMinus) {
        mTScaledPtSquared = computeMtScaledPtSquared();
        if (mTScaledPtSquared > 0)
          allHistograms.fill(HIST("SignalLoss/mTScaled_fromOmegaMinus"),
                             std::sqrt(mTScaledPtSquared), centralityPercent);
      } else if (pdgCode == -lambda_analysis::PdgOmegaMinus) {
        mTScaledPtSquared = computeMtScaledPtSquared();
        if (mTScaledPtSquared > 0)
          allHistograms.fill(HIST("SignalLoss/mTScaled_fromOmegaPlus"),
                             std::sqrt(mTScaledPtSquared), centralityPercent);
      }
    }
  }
