...
struct Lambda1520Analysispo {
  ...
  // FIX name/configurable: lowerCamelCase, member name == JSON string, on one line
  ...
  Configurable<bool> mcRequireRecoINELgt0{"mcRequireRecoINELgt0", false, "MC event selection: require reconstructed INEL>0 condition"};
                                   ...
                                   int occupancyValue = 100)
  {
    ...
    for (auto const& [protonTrack, kaonTrack] :
        ...
        if (protonTrack.sign() * kaonTrack.sign() < 0) {
          ...
          if (enableRotationalBackground) {
            for (int iRot = 0; iRot < numberOfRotations; iRot++) {
              // FIX magic-number: named constant for the half-window calculation
              ...
              // FIX two-pi-add-subtract: use RecoDecay::constrainAngle instead of manual +/- TwoPI
      ...
      if constexpr (isMCAnalysis) {
        if (protonTrack.sign() * kaonTrack.sign() < 0) {
          // FIX pdg/explicit-code: use named constants from lambda_analysis namespace
          if (std::abs(protonTrack.pdgCode()) != lambda_analysis::kPdgProton ||
              std::abs(kaonTrack.pdgCode()) != lambda_analysis::kPdgKaon)
            continue;
                 ...
                 aod::ResoMCParents const& mcParents)
  {
    ...
    for (const auto& track : tracks) {
      ...
      if (std::abs(track.pdgCode()) == lambda_analysis::kPdgKaon)
        allHistograms.fill(HIST("QAChecks/kaonGenLevelPt"), track.pt());
      if (std::abs(track.pdgCode()) == lambda_analysis::kPdgProton)
      ...
      if (passesKaonPID(track, totalMom) && std::abs(track.pdgCode()) == lambda_analysis::kPdgKaon)
        allHistograms.fill(HIST("QAChecks/kaonRecoLevelPt"), track.pt());
      if (passesProtonPID(track, totalMom) && std::abs(track.pdgCode()) == lambda_analysis::kPdgProton)
    ...
    for (const auto& parentParticle : mcParents) {
      ...
      // FIX pdg/explicit-code: use named PDG constants
      bool hasProtonDaughter = (std::abs(parentParticle.daughterPDG1()) == lambda_analysis::kPdgProton ||
                                std::abs(parentParticle.daughterPDG2()) == lambda_analysis::kPdgProton);
      bool hasKaonDaughter = (std::abs(parentParticle.daughterPDG1()) == lambda_analysis::kPdgKaon ||
                              std::abs(parentParticle.daughterPDG2()) == lambda_analysis::kPdgKaon);
                    ...
                    aod::ResoMCParents const& mcParents)
  {
    ...
    for (const auto& parentParticle : mcParents) {
      ...
      // FIX pdg/explicit-mass: use o2::constants::physics::MassLambda1520 (already done via using)
      auto computeMtScaledPtSquared = [&]() -> double {
        return (parentPt * parentPt) + (parentMass * parentMass) -
               (MassLambda1520 * MassLambda1520);
      };

      // FIX pdg/explicit-code: use named PDG constants from lambda_analysis namespace
      if (pdgCode == lambda_analysis::kPdgProton) {
      ...
      } else if (pdgCode == lambda_analysis::kPdgXiMinus) {
        mTScaledPtSquared = computeMtScaledPtSquared();
        if (mTScaledPtSquared > 0)
      ...
      } else if (pdgCode == lambda_analysis::kPdgXi0) {
        mTScaledPtSquared = computeMtScaledPtSquared();
        if (mTScaledPtSquared > 0)
      ...
      } else if (pdgCode == lambda_analysis::kPdgOmegaMinus) {
        mTScaledPtSquared = computeMtScaledPtSquared();
        if (mTScaledPtSquared > 0)
